...
function binarySearch(
  ...
  while (low < high) {
    const middle: number = (low + high + (isMaximum ? 1 : 0)) >>> 1;
    const result: number = filter(indices[middle]);
...
function getFiltered(
  ...
  const min: number = binarySearch(indices, filter, false);
  const max: number = binarySearch(indices, filter, true);

  return min >= 0 && max >= 0
    ? getSubArray(indices, min, max)
    : new Uint32Array(0);
}
...
function filterStrings(
  ...
  const len: number = searchInput.length;

  return getFiltered(sortedIndices, (index: number) => {
    const value: string = data[index].toLowerCase();
    ...
    for (let i: number = 0; i < len; i++) {
      ...
      const charA: string = value[i];
      const charB: string = searchInput[i];
